...
Water::Water(const std::string& name) {
	...
	Vector2 textureScale = Vector2(1 / 80.0f, 1 / 80.0f);

	for (int z = 0; z < iHeight; ++z) {
		for (int x = 0; x < iWidth; ++x) {
			int offset = (z * iWidth) + x;
			vertices[offset] = Vector3(x, 2.0, z) * vertexScale;
	...
	for (int z = 0; z < iHeight - 1; ++z) {
		for (int x = 0; x < iWidth - 1; ++x) {//how to match indices with vertices
			int a = (z * (iWidth)) + x;
			int b = (z * (iWidth)) + (x + 1);
	...
	WaterSize.x = vertexScale.x * (iWidth - 1);
	WaterSize.y = vertexScale.y * 255.0f;//each height is a b
	WaterSize.z = vertexScale.z * (iHeight - 1);
}

...
uniform sampler2D shadowTex_spot;
...
void main(void){
    ...
    // float distance = length(lightPos - IN.worldPos);
     ...
     //direction light no attenuation
     ...
     if(abs(shadowNDC.x) < 1.0f && abs(shadowNDC.y) < 1.0f && abs(shadowNDC.z) < 1.0f){
      ...
      vec3 surface = (diffuse.rgb *attenuation * ParlightColour.rgb);
     fragColour.rgb = surface* lambert;
     fragColour.rgb += (ParlightColour.rgb  * attenuation * specFactor) * 0.33;

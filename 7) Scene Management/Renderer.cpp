...
Renderer::Renderer(Window& parent) :OGLRenderer(parent) {
	camera = new Camera(0.0f, 0.0f, Vector3(0.0f, 100.0f, 750.0f));
...
void Renderer::UpdateScene(float dt) {
	...
	frameFrustum.FormMatrix(projMatrix * viewMatrix);
...
void Renderer::BuildNodeLists(SceneNode* from) {
	if (frameFrustum.InsideFrustum(*from)) {
		...
		//from->SetCameraDistance(sqrt(Vector3::Dot(dir, dir)));
...
void Renderer::SortNodeLists() {
	std::sort(transparentNodelist.rbegin(), transparentNodelist.rend(), SceneNode::CompareByCameraDistance);//note reverse iterator
